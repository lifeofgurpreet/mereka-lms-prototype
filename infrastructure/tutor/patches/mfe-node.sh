...
PY"""
    ...
    def ensure_mfe_account_compiled_social_links_guard(text):
        guard_line = 'guard_revision = "account-social-links-guard-2026-04-04-cacheproof-v1"'
        if "FROM account-common AS account-prod" not in text:
            return text
        if guard_line in text and "unguarded social_links lookup survived account build" in text:
            return text

        build_assert_block = """RUN python3 - <<'PY'
from pathlib import Path
...
PY"""
        ...
        start = None
        for idx, line in enumerate(lines):
            if line.strip() == "FROM account-common AS account-prod":
                start = idx
                break
        if start is None:
            return text
        ...
        lines = lines[:insert_at] + [build_assert_block] + lines[insert_at:]
        rebuilt = "\n".join(lines)
        if text.endswith("\n"):
            rebuilt += "\n"
        return rebuilt
    ...
    updated = ensure_mfe_account_compiled_social_links_guard(updated)
