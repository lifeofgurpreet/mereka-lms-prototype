...
class TestMiddlewarePathFiltering(unittest.TestCase):
    ...
    def test_view_owned_response_header_skips_processing(self):
        factory = RequestFactory()
        request = factory.get("/api/mfe_context")
        response = HttpResponse(
            json.dumps({"contextData": {"providers": []}}).encode("utf-8"),
        ...
        with patch(
            "mfe_oauth_fix.middleware.json.loads",
...
class TestMiddlewareAuthentikRenaming(unittest.TestCase):
                ...
                "providers": [
                    {
                        "id": "oa2-sso",
                        "name": "Authentik SSO",
                        "loginUrl": "/auth/login/oauth2/",
                        "registerUrl": "/auth/login/oauth2/",
                    }
                ]
            }
        }
                ...
                "providers": [
                    {
                        "id": "oa2-authentik",
                        "name": "SSO Login",
                        "loginUrl": "/auth/login/x/",
                        "registerUrl": "/auth/login/x/",
                    }
                ]
            }
        }
    ...
    def test_handles_none_provider_fields(self):
        ...
        data = {
            "contextData": {
                "providers": [{"id": None, "name": None, "loginUrl": None, "registerUrl": None}]
            }
        }
        request = self.factory.get("/api/mfe_context")
        response = self._make_response(data)
...
class TestMiddlewareEmptyProviders(unittest.TestCase):
    ...
    def test_empty_providers_triggers_lookup(self):
        ...
        with (
            patch("mfe_oauth_fix.middleware.get_current_site") as mock_site,
            patch("mfe_oauth_fix.middleware.OAuth2ProviderConfig", create=True),
        ):
...
class TestMFEContextView(TestCase):
    ...
    def test_returns_json_with_providers_key(self):
        ...
        self.assertEqual(response[MFE_CONTEXT_SOURCE_HEADER], MFE_CONTEXT_SOURCE_VIEW)
