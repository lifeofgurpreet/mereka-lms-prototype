...
POLICY_PATH = REPO_ROOT / "verification" / "manifests" / "ci_failure_severity_policy.json"
POLICY_SCHEMA_PATH = REPO_ROOT / "schemas" / "ci-failure-severity-policy.schema.json"
...
def test_policy_file_is_schema_valid() -> None:
    payload = json.loads(POLICY_PATH.read_text(encoding="utf-8"))
    jsonschema.validate(payload, load_policy_schema())
...
def test_compute_decision_distinguishes_branch_failures(tmp_path: Path) -> None:
    ...
    assert payload["new_failure_details"][0]["severity"] == "medium"
...
def test_compute_decision_recognizes_baseline_only_failures(tmp_path: Path) -> None:
    ...
    assert payload["shared_failure_details"][0]["merge_policy"] == "review_required_if_baseline"
    jsonschema.validate(payload, load_schema())


def test_compute_decision_blocks_on_high_severity_baseline_debt(tmp_path: Path) -> None:
    module = load_module()
    policy = module.load_policy(POLICY_PATH)
    branch_dir = tmp_path / "branch"
    baseline_dir = tmp_path / "baseline"
    ...
    write_log(
        branch_dir / "Static Validation Scripts (shard-02).txt",
        [
            "FAIL verify-secret-classification [9/200] (exit 1, 0s)",
        ],
    )
    write_log(
        baseline_dir / "Static Validation Scripts (shard-02).txt",
    ...
    assert payload["decision"] == "blocked_on_baseline_policy"
    assert payload["blocking_shared_failures"] == ["verify-secret-classification"]
    assert payload["shared_failure_details"][0]["severity"] == "high"
    jsonschema.validate(payload, load_schema())
...
def test_cli_emits_schema_valid_payload(tmp_path: Path) -> None:
    ...
    assert payload["decision"] == "mergeable_with_baseline_debt"
    assert payload["policy"]["schema_version"] == "ci-failure-severity-policy/v1"
    jsonschema.validate(payload, load_schema())
