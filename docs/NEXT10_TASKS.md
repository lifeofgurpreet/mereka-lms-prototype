...
| 3 | Budget & cost guardrails (RM250 alert, RM400 cap) | FinOps | ⚙️ In progress | Terraform now provisions `google_billing_budget.mereka_monthly` with 62.5%/100% thresholds (see `ops/terraform/budgets.tf`). Needs someone with billing-admin rights to run `terraform apply -target=google_billing_budget.mereka_monthly` so the alerts go live. |
