...
- `scripts/qa/verify-mfe-config-contract.sh --env {prod|dev|both}`
  - Verifies `/api/mfe_config/v1` on the public apps host contains the auth-critical keys we depend on:
    ...
    - `REFRESH_ACCESS_TOKEN_ENDPOINT` MUST be **same-origin** (absolute or relative):
      - `https://apps.<domain>/login_refresh`
      - `/login_refresh` (preferred; robust across multiple MFE hostnames)
...
- `./scripts/gen/update-openedx-hostnames-doc.sh` regenerates `docs/reference/operations/OPENEDX_HOSTNAMES.md`.
