...
| Local authenticated runtime readiness | `verify-local-runtime-readiness.sh` against the initialized Tutor root being claimed | LMS/Discovery shell access and local proof identity completeness are established for authenticated smoke work. | It is not first-boot, image-build, or browser-flow proof. |
