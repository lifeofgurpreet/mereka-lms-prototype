...
| cold-start onboarding | `verify-cold-start-onboarding-contract.sh`, `bootstrap-local-readiness.yml`, `build-benchmark.yml` | local setup/source-guide consistency, first-run Tutor bootstrap, app-cache-cold image build | onboarding docs, setup scripts, Tutor render/build helpers | repo source, rendered Tutor config, selected benchmark class | local contract output, bootstrap artifact, benchmark artifact | docs-only pass can miss full Tutor launch; app-cache-cold is not pristine daemon proof | runner capacity or registry pull transients | platform | Developer environment proof matrix |
| developer environment matrix | `verify-cold-start-onboarding-contract.sh` plus lane-specific verifiers | source/render/artifact/cache authority for local, CI, devspace, and preview lanes | developer environment contracts | lane matrix row + linked verifier | contract output and lane proof artifact | new lanes can look supported without a proof row | planned lanes fail until implemented | platform | n/a |
