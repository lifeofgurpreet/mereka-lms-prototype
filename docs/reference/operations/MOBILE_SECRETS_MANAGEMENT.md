...
> **Current posture (2026-04-09)**: iOS remains a major intended product surface, but the active delivery lane is currently paused for cost/program timing. Android remains deferred. Treat this document as the maintained future mobile secret contract, not proof that mobile CI is currently running.
> **Start here**: Use [`IOS_CI_CD_REFERENCE.md`](IOS_CI_CD_REFERENCE.md) and [`../../ops/runbooks/MOBILE_DEPLOYMENT.md`](../../ops/runbooks/MOBILE_DEPLOYMENT.md) first to confirm the current lane status before rotating or auditing mobile secrets.
> **Owner boundary (2026-04-10)**: Do not create a separate Android or Google Play release-reference companion while Android remains deferred. Current owner split is: iOS workflow contract here and in `IOS_CI_CD_REFERENCE.md`; paused-lane runtime and resumed-ops boundary in `../../ops/runbooks/MOBILE_APPS_RUNBOOK.md`.
...
| Secret Name | Bridge Key Name | Infisical Path | Purpose | Distribution |
|-------------|------------|-----------------|---------|-------------|
...
| Secret Name | Bridge Key Name | Infisical Path | Purpose | Distribution |
|-------------|------------|-----------------|---------|-------------|
| App Store Connect API Key | `MEREKA_LMS_APPSTORE_CONNECT_KEY_ID` | `/k8s/mereka-lms/mobile` | TestFlight upload automation | CI/CD (current iOS lane when resumed) |
| App Store Connect Issuer ID | `MEREKA_LMS_APPSTORE_CONNECT_ISSUER_ID` | `/k8s/mereka-lms/mobile` | TestFlight upload automation | CI/CD (current iOS lane when resumed) |
| App Store Connect Key Content | `MEREKA_LMS_APPSTORE_CONNECT_KEY` | `/k8s/mereka-lms/mobile` | TestFlight upload automation | CI/CD (current iOS lane when resumed) |
| Match Git Repository | `MEREKA_LMS_MATCH_GIT_URL` | `/k8s/mereka-lms/mobile` | Historical iOS code signing lane context | Legacy Fastlane context / future reassessment only |
| Match Repository Password | `MEREKA_LMS_MATCH_PASSWORD` | `/k8s/mereka-lms/mobile` | Historical iOS code signing lane context | Legacy Fastlane context / future reassessment only |
...
### 1. Infisical to the Secret Manager bridge

All mobile secrets are stored in Infisical under the path `/k8s/mereka-lms/mobile` and propagated through the current Secret Manager bridge using the standard sync script:

```bash
# Sync mobile secrets through the current bridge implementation
./scripts/infra/sync-mereka-lms-secrets-to-gcpsm.sh

# Verify bridge entries exist (production implementation detail: GCP Secret Manager)
gcloud secrets list --filter="name~MEREKA_LMS_MOBILE_" --format="table(name,createTime)"
```

### 2. Bridge to ExternalSecrets
...
### 3. Bridge to GitHub Actions

CI/CD workflows retrieve secrets from the current bridge implementation using Workload Identity Federation:

```yaml
# Current iOS workflow family (confirm exact active workflow in IOS_CI_CD_REFERENCE.md)
...
**iOS (Xcode)**:
- Secrets are injected into `Config.xcconfig` via the active iOS CI lane in force at build time
...
## Firebase and APNs Setup Procedures

Use this section when re-validating the paused notification lane before mobile
reactivation.

### Firebase setup

1. confirm the intended Firebase project for the resumed lane
2. retrieve the current iOS plist / Android json from the canonical secret
   authority, not from local developer machines
3. verify the build lane injects the correct file for the intended environment
4. verify the Firebase project matches the bundle/application identifiers in
   use

### APNs / iOS notification setup

1. confirm the Apple bundle ID still matches the intended iOS production app
2. confirm the APNs key/certificate path in Firebase is still valid
3. confirm the iOS app is using the intended Firebase project before expecting
   token registration to succeed

### Google Play service-account setup

Use this only for resumed Android delivery work.

1. confirm the Play Console app/package name still matches the planned Android
   surface
2. confirm the current service-account json is present in the canonical secret
   authority
3. confirm the CI lane or manual release flow injects the service account only
   at build/release time
4. do not treat presence of the Play service-account secret as proof that
   Android delivery is active

### Android release-reference boundary

Android remains deferred. Until that changes:

- do not create a separate Android release-reference doc
- keep Google Play and Android signing/distribution boundary notes here
- keep resumed-lane operational procedure in
  [../../ops/runbooks/MOBILE_APPS_RUNBOOK.md](../../ops/runbooks/MOBILE_APPS_RUNBOOK.md)
- treat a future standalone Android release reference as warranted only if the
  Android delivery lane becomes an intended maintained surface with a stable CI
  owner distinct from the paused mobile posture
...
# 5. Verify the bridge implementation received the new value
gcloud secrets versions access latest --secret=MEREKA_LMS_MOBILE_IOS_PROD_CLIENT_SECRET

# 6. Trigger the currently active iOS workflow to pick up the rotated secret
# Confirm the exact workflow first in docs/reference/operations/IOS_CI_CD_REFERENCE.md
...
### Secret Manager Bridge Audit Logs

The current bridge implementation maintains audit logs for all secret access operations.
...
gh run list --workflow=ios-testflight.yml --limit 10 --json databaseId,conclusion,createdAt
...
Verify all mobile secrets exist in Infisical and the current Secret Manager bridge:
...
# ✓ All 16 mobile secrets synced to the current bridge implementation
...
5. **Audit access**: the current bridge implementation's audit logs track all secret access operations.
...
### Mobile Secrets Missing from the Secret Manager Bridge

**Symptom**: CI build fails with "secret not found" error.

**Diagnosis**:
```bash
# Check if secrets exist in the current bridge implementation
...
**Resolution**: Run `./scripts/infra/sync-mereka-lms-secrets-to-gcpsm.sh` to sync from Infisical through the current Secret Manager bridge.
