...
_Audience: Operators and reviewers • Owner: Platform Team • Last verified: 2026-04-14 • Status: canonical_
...
This document is the canonical operations reference for the UI/UX hardening lane. It defines:
...
```
  FAIL  AC-HB-001: visual regression baseline section missing from docs/reference/operations/UI_UX_HARDENING_BUNDLE.md
```

The failure message must:
- Identify the AC (e.g. `AC-HB-001`)
- State what was expected (e.g. "section missing")
- State where to fix it (e.g. "docs/reference/operations/UI_UX_HARDENING_BUNDLE.md")
...
python3 -c "
import re, datetime
text = open('docs/reference/operations/UI_UX_HARDENING_BUNDLE.md').read()
