...
2. **Render proof**: `./scripts/infra/tutor-config-save.sh` + `./scripts/infra/verify-tutor-config.sh` passes
...
- **Never edit `tutor_env/` files directly** — they are regenerated by the governed Tutor config wrapper
...
- **Never skip `prepare-tutor-build-context.sh --target mfe`** after `./scripts/infra/tutor-config-save.sh` — it is the governed wrapper that refreshes the rendered MFE build context and runs the bounded patch helper path
