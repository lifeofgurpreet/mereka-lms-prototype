...
- Canonical logging + Sentry runbook: `docs/ops/monitoring/LOGGING_AND_SENTRY.md`.
- Fast operator flow: `docs/ops/monitoring/OBSERVABILITY_QUICKSTART.md`.
- Ownership model: `docs/ops/monitoring/OBSERVABILITY_OWNERSHIP.md`.
...
3. **Data plane recovery** – for in-cluster state (MySQL/Redis PVs), recovery depends on Velero snapshot+restore drills. See `docs/ops/runbooks/DISASTER_RECOVERY.md`.
...
6. **Auth alert remediation** – see `docs/ops/runbooks/AUTH_ALERT_RUNBOOK.md` for a mapping from each auth alert to the exact verification and fix commands.
