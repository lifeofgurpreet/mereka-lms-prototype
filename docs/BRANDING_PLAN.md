...
- Runtime evidence (`#105`):
  ...
  - Latest live cluster audit: `./scripts/qa/verify-credentials-readiness.sh --cluster` -> `PASS=53 FAIL=3 SKIP=0` (`var/qa/credentials-readiness-cluster-20260302T102541Z.log`), including explicit runtime failure for `ZoneInfo('UTC')` (`ModuleNotFoundError: No module named 'tzdata'`), confirming dev credentials runtime has not yet converged.
