...
| **Plugin file splitting** | Extract 47 React component strings from `mereka_lms.py` into separate file(s). Status: staged/in-progress (`#109` phase 1 + phase 2 + phase 3 + phase 4 + phase 5 + phase 6 + phase 7 + phase 8 complete; verifier compatibility layer expanded, QA coupling reduced 98→28). See `docs/operations/PLUGIN_SPLIT_STATUS_2026-03-02.md`. |
