...
- Runtime evidence (`#105`):
  ...
  - Latest live cluster audit: `./scripts/qa/verify-credentials-readiness.sh --cluster` -> `PASS=54 FAIL=2 SKIP=0` (`var/qa/credentials-readiness-cluster-20260302T103112Z.log`), with remaining runtime failures on DID endpoint response and `ZoneInfo('UTC')` (`ModuleNotFoundError: No module named 'tzdata'`), confirming dev credentials runtime has not yet converged.
