...
| OBS-006 | P0 | Coverage | Audit missing ServiceMonitor coverage vs required services in each env | OBS-005 | env gap report | report lists missing/extra monitors with owners and dates | planned |
...
| OBS-012 | P0 | Alert quality | Define severity-specific noise thresholds (`critical`, `error`, `warning`) | OBS-011 | updated baseline config | baseline schema includes per-severity thresholds | planned |
...
| OBS-020 | P1 | SLO | Add SLO breach runbook links in alert annotations | OBS-018 | alert annotation updates | each SLO alert links to concrete remediation runbook | planned |
...
| OBS-036 | P1 | Dashboards | Refactor dashboard contract to include ownership tags per panel | OBS-009 | contract schema update | each required panel has owner metadata | planned |
| OBS-037 | P1 | Dashboards | Add panel freshness audit (query returns data within N min) | OBS-036 | audit script update | audit fails on stale/no-data required panels | planned |
