...
if [ -n "$POLICY_RANGE" ]; then
  policy_cmd=(./docs/qa/verify-docs-policy.sh --range "$POLICY_RANGE" --summary-json "$policy_summary_json")
else
  policy_cmd=(./docs/qa/verify-docs-policy.sh --summary-json "$policy_summary_json")
fi
