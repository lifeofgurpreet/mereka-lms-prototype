...
| P1 | `mereka-lms-0z5g.23` | #2043 | in progress | App producer schema is reconciled, live receiver health was recovered, and vps-infrastructure PR #116 is merged/deployed. Close only after a post-restart build-metrics push proves cache/layer Prometheus samples use receiver-owned runner-context enrichment instead of `runner_class="other"` fallback. |
