...
|---|---|---|---|
| Current app `main` source | `4c0322c37519b9d21d80ad19ff839d0a946c0475` | source | Latest fetched `origin/main`; this source includes #2130 and #2133 after the benchmark maintenance-window guard. |
...
| Bootstrap local readiness | Run `24872614377`, commit `799b8d52dfba1f1ae307b1b45aba4339c93012e8` | CI local/bootstrap | Current accepted bootstrap proof is green and the proven commit is an ancestor of current `origin/main`. This proves clean repo-scoped Tutor bootstrap readiness; it is not image-build, browser-flow, or GitOps proof. |
...
| Fastlane prune-lock source/live proof | bbi-infrastructure PR #3959, merge `a2771950b431ae571f0b7c528fa7aef47f2e1a68`; host `vmi3220759` deployed cleanup hash `e4bee7847f2dc12aafbd860dacbfcd222db862abd55733605bad51401c1b8ae5` | infra source plus host runtime | Per-user prune-lock fallback is removed from source. Host lock `/tmp/fastlane-docker-prune.lock` is `0666 root:root`, scheduled hygiene is installed and active, dry-run hygiene passed, and an unwritable-lock smoke as `gha-build` failed closed without fallback. Real concurrent fastlane job proof is still blocked by `fallback-queue-full`. |
...
| P0 | `mereka-lms-0z5g.5` | #1777, #1778 | in progress | Fastlane/containerd/Buildx hygiene has source and host proof; keep open until real fastlane job evidence confirms the post-job hook runs without prune-lock fallback after the current queue gate clears. |
| P1 | `mereka-lms-0z5g.5.1` | #1778 | open | `/tmp/fastlane-docker-prune.lock` ownership is versioned and host-proven. Final closure still needs concurrent real fastlane jobs; dispatch attempts `24878999783` and `24879000904` failed closed at runner selection with `fallback-queue-full`. |
...
4. Advance `mereka-lms-0z5g.5` and `.5.1`: wait for fastlane queue depth to
   drop below the selector threshold, rerun two lightweight scan-only fastlane
   jobs, and close only if real job logs show no prune-lock fallback.
