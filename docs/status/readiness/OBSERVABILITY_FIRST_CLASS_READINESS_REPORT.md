...
- canonical_index: `verification/catalogs/OBSERVABILITY_CANONICAL_INDEX.md`
...
Canonical index:
- `verification/catalogs/OBSERVABILITY_CANONICAL_INDEX.md`
...
Acceptance basis: release readiness semantics captured in tracker and documented in `docs/policies/operations/OBSERVABILITY_OWNERSHIP.md`.
...
- Upgraded `scripts/qa/validate-observability-compliance.sh --mode runtime` to apply the same `/metrics` validity contract as runtime checks, including payload-shape validation (`# HELP`/`# TYPE` + numeric sample checks) for LMS/CMS.
...
- Added canonical parity matrix: `docs/reference/operations/OBSERVABILITY_PARITY_MATRIX.md`.
