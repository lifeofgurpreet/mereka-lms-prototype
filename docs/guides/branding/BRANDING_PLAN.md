...
_Audience: Design + Platform Eng • Owner: Branding Guild • Last verified: 2026-03-03 • Status: canonical_
...
- Runtime evidence (`#105`):
  ...
  - Deterministic verification hardening in this tranche:
    ...
    - latest strict readiness capture rerun passed for closure routes: `CAPTURE_STRICT_READY=1 CAPTURE_RETRIES=2 AGENT_BROWSER_TIMEOUT_SECONDS=45 ./scripts/qa/capture-branding-screenshots.sh --env dev --core-routes` -> `var/screenshots/dev/20260303T221620Z/capture-summary.tsv` (log `var/qa/capture-branding-screenshots-dev-core-strict-20260303T221620Z.log`).
...
- Root-cause hardening applied in repo:
  ...
  - Detailed stabilization log: `docs/archive/reports/FRONTEND_RUNTIME_STABILITY_STATUS_2026-03-02.md`.
  - Issue closure matrix: `docs/archive/reports/FRONTEND_CLOSURE_STATUS_MATRIX_2026-03-02.md`.
...
## Inputs To Confirm
- [x] Primary/secondary hex palette (captured in `./BRANDING.md#palette`).
...
> See [FRONTEND_PHASE_C_PROMPT.md](../../archive/FRONTEND_PHASE_C_PROMPT.md) and [deep audit](../../archive/reports/reviews/FRONTEND_PHASE_AB_DEEP_AUDIT.md).
...
- [x] **Fix OEP-48 mandatory file gaps** (4 missing files) — see [deep audit §4](../../archive/reports/reviews/FRONTEND_PHASE_AB_DEEP_AUDIT.md#4-missing-oep-48-mandatory-files-confirmed-from-audit-v1)
...
- [x] **Split theme.scss for MFE consumption** — `mereka.scss` now imports focused MFE partials instead of full LMS/Studio theme — see [deep audit §3 CRIT-2](../../archive/reports/reviews/FRONTEND_PHASE_AB_DEEP_AUDIT.md#crit-2-themescss-leaks-600-lines-of-lmsstudio-css-into-every-mfe)
- [x] **Fix runtime theme CSS bloat** — `mereka-brand.min.css` now generated as brand delta (1.9KB), `light.min.css` as light-variant delta — see [deep audit §3 CRIT-3](../../archive/reports/reviews/FRONTEND_PHASE_AB_DEEP_AUDIT.md#crit-3-runtime-theme-css-files-are-bloated-and-duplicated)
...
> **Architecture note (2026-02-28)**: Brand package `_variables.scss` is **dead in Ulmo** — Paragon v23+ ignores SCSS variables. Our actual theming works through `mereka.scss` → `_tokens.scss` CSS custom properties. See [deep audit §3 CRIT-1](../../archive/reports/reviews/FRONTEND_PHASE_AB_DEEP_AUDIT.md#crit-1-brand-package-scss-variables-are-dead).
