...
2) **Incomplete LB exposure (missing HTTPS port)**  
   - Caddy Service lacked port 443, so LB never opened TLS. Users hitting `https://staging.academy.mereka.io` saw timeouts even once selectors were corrected.  
...
1. **If login fails (CSRF 403/500): ensure Auth MFE + cookies**
   ...
   #   LOGIN_MICROFRONTEND_URL = LOGISTRATION_MICROFRONTEND_URL = https://apps.staging.academy.mereka.io/authn
...
5. **Lock in guardrails (prevents recurrence)**
   ...
   - Add a small canary/uptime probe (HTTPS) hitting `staging.academy.mereka.io` and alert on 5xx/timeouts.
...
- `curl -Ik https://staging.academy.mereka.io` → `HTTP/2 200` (or 308 redirect to HTTPS)  
