...
| Document | Content |
|----------|---------|
| `docs/guides/branding/MULTI_TENANT_BRANDING_OPS.md` | Per-tenant onboarding runbook (Step-by-step); branding layers; CI validation; owner/signoff/escalation |
| `docs/guides/branding/TENANT_BRANDING_CONTRACT.md` | Required inputs, brand pack schema, fallback rules |
| `docs/guides/branding/TENANT_BRAND_PACK_SCHEMA.md` | JSON schema for tenant brand pack (logo, colors, fonts, domains) |
| `docs/guides/branding/BRANDING_OPERATING_MODEL.md` | Canonical branding workflow, change types, approvals |
| `docs/ops/runbooks/tenant-provisioning-runbook.md` | Operator runbook for K8s provisioning |
...
|------|------|---------|-------|
| Multi-tenant architecture | `specs/multi-tenancy-architecture_spec.md` | `docs/ops/runbooks/tenant-provisioning-runbook.md` | platform-engineering |
| Branding operations | `specs/platform-middleware-custom-apps_spec.md` | `docs/guides/branding/BRANDING_OPERATING_MODEL.md` | platform-engineering / design |
| Branding contract | (covered in multi-tenancy spec) | `docs/guides/branding/TENANT_BRANDING_CONTRACT.md` | platform-engineering |
...
- Automated screenshot diffing (pixel-by-pixel comparison) is not yet implemented. Current coverage relies on HTTP check + CSS marker verification. Perceptual diff tooling (e.g., Percy, Chromatic) is in `docs/guides/branding/BRANDING_ROADMAP.md` as a future item.
