...
The floor is a gate, not a vanity metric.  A spec AC with no test reference is
unverified intent — it may already be broken in production and nobody would know.
...
2. **Count ACs per spec** — applies heuristics in priority order:
   - Checkbox lines (`- [ ]` / `- [x]`) — preferred; most specs use this style.
...
Lines marked `<FLOOR` are spec files whose individual coverage falls below the
configured floor.  They do not fail the check independently — only the overall
aggregate is enforced — but they are highlighted in red to guide remediation work.
...
(or in any Makefile target that invokes this script) **and** in this table so
the intent is documented.

**Never lower the floor** unless a spec is intentionally removed or split.
Lowering the floor to make CI green is a process violation — fix the coverage gap instead.
...
4. Ensure the referenced file actually exists and exits 0 on success.
5. Re-run `./scripts/qa/verify-spec-coverage.sh` to confirm coverage improves.
...
The script exits 1 when coverage is below the floor, which will fail the workflow job.
