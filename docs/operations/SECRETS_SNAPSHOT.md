...
_Audience: Platform Eng • Owner: Infra Team • Last verified: 2025-08-22_
...
| Cloudflare global API key | Stored in 1Password under “Cloudflare – gurpreet@biji-biji.com”. Export as `CLOUDFLARE_API_KEY` only when running `cli4`/`scripts/infra/cloudflare-sync.sh`, then rotate if shared. |
