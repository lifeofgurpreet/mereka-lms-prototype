...
- Latest repo-only stabilization update (while infra sync pending):
  ...
  - Latest live cluster evidence (`var/qa/credentials-readiness-cluster-20260302T102541Z.log`) reports `PASS=53 FAIL=3 SKIP=0`, including failed runtime `ZoneInfo('UTC')` and missing python `tzdata`, so credentials runtime rollout convergence is still pending.
