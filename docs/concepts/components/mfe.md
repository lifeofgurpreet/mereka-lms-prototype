# MFE Component Index
_Audience: Frontend Engineers and Operators • Owner: Platform Team • Last verified: 2026-03-06 • Status: canonical_

## What This Component Is
MFEs are the learner/admin frontend surfaces and associated plugin slots, asset pipelines, and release controls.

## Canonical ADR Links
- [ADR 014: MFE branding strategy](../../programs/frontend/MFE_BRANDING_MIGRATION_DECISION.md)
- [ADR 021: Open edX Tutor methodology](../../adr/021-openedx-tutor-methodology.md)

## Canonical Operational Runbooks
- [MFE Plugin Slots Runbook](../../ops/runbooks/MFE_PLUGIN_SLOTS_RUNBOOK.md)
- [Module Surface Validation Runbook](../../ops/runbooks/MODULE_SURFACE_VALIDATION_RUNBOOK.md)
- [CI/CD Runbook](../../ops/runbooks/CI_CD_RUNBOOK.md)

## Canonical Guides
- [MFE Complete List](../../reference/architecture/MFE_COMPLETE_LIST.md)
- [Branding Guide](../../guides/branding/BRANDING.md)
- [Footer v2 to LMS Mapping](../../reference/architecture/FOOTER_V2_TO_LMS_MAPPING.md)
- [Enterprise MFE Runtime Config Truth Contract](../../runtime-proof/ENTERPRISE_MFE_RUNTIME_CONFIG_TRUTH_CONTRACT.md)

## Active CI/Workflow References
- [Build Tutor Images Workflow](../../../.github/workflows/build-tutor-images.yml)
- [CI Workflow](../../../.github/workflows/ci.yml)
