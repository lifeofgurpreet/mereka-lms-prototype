# Foundations Bundle

> Generated file. Do not hand-edit. Regenerate from the ADR source inputs.

This file is generated from `docs/adr/manifest.yaml`.

## ADRs

- `ADR-018` [Purchase Gateway Replaces Legacy Oscar Ecommerce](../../docs/adr/018-purchase-gateway-replaces-oscar-ecommerce.md)
  - Governs: `commerce.system-of-record, commerce.reconciliation, platform.change-policy`
- `ADR-019` [ADR-019: Tutor Upgrade Cadence and EOL Policy](../../docs/adr/019-tutor-upgrade-policy.md)
  - Governs: `n/a`
- `ADR-021` [ADR-021: Open edX / Tutor Deployment Methodology](../../docs/adr/021-openedx-tutor-methodology.md)
  - Governs: `n/a`
- `ADR-024` [ADR-024: True Multi-Tenancy for Subsites (Biji-Biji, SkillOurFuture)](../../docs/adr/024-multi-tenancy-true-tenants.md)
  - Governs: `n/a`
- `ADR-028` [Platform Sources of Truth and Control Planes](../../docs/adr/028-platform-sources-of-truth-and-control-planes.md)
  - Governs: `platform.control-plane, platform.repo-boundary, build.gitops-promotion`
- `ADR-029` [Identity, Session, and Domain-Boundary Strategy](../../docs/adr/029-identity-session-and-domain-boundary-strategy.md)
  - Governs: `auth.oidc, auth.cookie-boundary, tenant.domain-boundary`
- `ADR-030` [Feature Flag and Rollout Lifecycle](../../docs/adr/030-feature-flag-and-rollout-lifecycle.md)
  - Governs: `platform.change-policy`
- `ADR-031` [Deprecation and Removal Policy](../../docs/adr/031-deprecation-and-removal-policy.md)
  - Governs: `platform.change-policy`
- `ADR-032` [Data Governance, PII, Retention, and Deletion](../../docs/adr/032-data-governance-pii-retention-and-deletion.md)
  - Governs: `data.pii, data.retention, data.deletion`
- `ADR-033` [Tenant Lifecycle Contract](../../docs/adr/033-tenant-lifecycle-contract.md)
  - Governs: `tenant.lifecycle, tenant.isolation, tenant.domain-boundary, frontend.brand.tokens`
