...
def main():
    ...
    subprocess.run(['kubectl', 'cp', 'ops/migrations/kajabi/scripts/openedx_bulk_import.py', 
                   f'{namespace}/{lms_pod}:/tmp/openedx_bulk_import.py'], check=True)
    subprocess.run(['kubectl', 'cp', 'ops/migrations/mct/output/openedx/users_import_sanitized.csv', 
                   f'{namespace}/{lms_pod}:/tmp/mct-users.csv'], check=True)
    subprocess.run(['kubectl', 'cp', 'ops/migrations/mct/output/openedx/enrollments_import.csv', 
                   f'{namespace}/{lms_pod}:/tmp/mct-enrollments.csv'], check=True)
    print("✅ Files copied")
    
    # Import users
    users_imported, users_failed = import_users_batched(namespace, lms_pod, 
                                                       Path('ops/migrations/mct/output/openedx/users_import_sanitized.csv'),
    ...
    enrollments_imported, enrollments_failed = import_enrollments_batched(namespace, lms_pod,
                                                                         Path('ops/migrations/mct/output/openedx/enrollments_import.csv'),
