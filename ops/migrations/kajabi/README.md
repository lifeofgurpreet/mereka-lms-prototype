...
- `ops/migrations/kajabi/output/course_packages/<slug>/<slug>.tar.gz` – ready for Studio import.
- `ops/migrations/kajabi/output/course_packages/course_packages_manifest.csv` – maps every Kajabi course ID to the generated course key (`course-v1:ORG+NUMBER+RUN`) plus module/lesson counts.
...
Outputs land in `ops/migrations/kajabi/output/openedx/`:
...
```
TARBALL=ops/migrations/kajabi/output/course_packages/ai-fluency-for-corporates/ai-fluency-for-corporates.tar.gz
...
```
source ops/tutor-env.sh
python ops/migrations/kajabi/scripts/import_courses.py \
  --manifest ops/migrations/kajabi/output/course_packages/course_packages_manifest.csv \
