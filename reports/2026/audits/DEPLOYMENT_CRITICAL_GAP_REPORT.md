...
| k8s-deployment AC-INT-001..005 | Require live cluster for pod/secret inspection | P2 | Covered by `scripts/qa/verify-k8s-live-cluster.sh` (and `scripts/qa/verify-k8s-deployment-spec.sh`) when `KUBECONFIG` is present; add `type: manual` entries pointing to `docs/runbooks/operations/TROUBLESHOOTING.md` |
