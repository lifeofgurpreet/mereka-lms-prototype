...
# Usage: ./tools/setup-google-oauth.sh [--client-id CLIENT_ID] [--client-secret SECRET]
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
source "$REPO_ROOT/ops/tutor-env.sh"
...
echo ""
echo "Next steps:"
echo "1. Restart the LMS service:"
echo "   tutor local restart lms    # for local"
echo "   tutor k8s restart lms     # for Kubernetes"
echo ""
echo "2. Enable the Google provider in Django admin:"
echo "   - Navigate to: http://localhost/admin/ (or https://staging.academy.mereka.io/admin/)"
