...
if atlas clusters describe "$CLUSTER_NAME" --projectId "$PROJECT_ID" >/dev/null 2>&1; then
  ...
  atlas clusters describe "$CLUSTER_NAME" --projectId "$PROJECT_ID" --output json | \
    jq '{name: .name, state: .stateName, tier: .replicationSpecs[0].regionConfigs[0].electableSpecs.instanceSize, region: .replicationSpecs[0].regionConfigs[0].regionName}'
...
log "Note: This requires payment information to be set up in Atlas."
atlas clusters create "$CLUSTER_NAME" \
  ...
  --diskSizeGB 10 \
  --mdbVersion 7.0 \
  --output json > /tmp/atlas-cluster-create.json

CLUSTER_STATUS=$(jq -r '.stateName' /tmp/atlas-cluster-create.json)
...
log ""
log "=========================================="
log "Atlas M10 cluster provisioned successfully!"
log "=========================================="
log ""
...
log "   ATLAS_URI='$ATLAS_URI' ./tools/mongodb-atlas-cutover.sh"
log ""
log "3. After verification, clean up the StatefulSet:"
log "   CLEANUP_STATEFULSET=true ATLAS_URI='$ATLAS_URI' ./tools/mongodb-atlas-cutover.sh"
log ""

# Save URI to a temporary file
