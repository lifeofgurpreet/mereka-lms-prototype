...
echo "Check 11/13: concepts architecture root is clean"
...
echo "Check 12/13: reference root has no self-stub docs"
...
echo "Check 13/13: guides root hygiene"
guides_hygiene_status="pass"
if ! python3 tools/docs/verify/verify_guides_hygiene.py --repo-root .; then
  guides_hygiene_status="fail"
fi

status="pass"
if [[ "$failures" -gt 0 ]] || [[ "$content_status" = "fail" ]] || [[ "$legacy_arch_status" = "fail" ]] || [[ "$legacy_ops_status" = "fail" ]] || [[ "$legacy_ci_cd_status" = "fail" ]] || [[ "$legacy_branding_status" = "fail" ]] || [[ "$legacy_runbooks_status" = "fail" ]] || [[ "$legacy_migrations_status" = "fail" ]] || [[ "$concepts_architecture_status" = "fail" ]] || [[ "$reference_self_stubs_status" = "fail" ]] || [[ "$guides_hygiene_status" = "fail" ]]; then
  status="fail"
fi

if [[ -n "$SUMMARY_JSON" ]]; then
  python3 - "$SUMMARY_JSON" "$RANGE" "$failures" "$content_status" "${#changed_md[@]}" "$CONTENT_SUMMARY_JSON" "$legacy_arch_status" "$legacy_ops_status" "$legacy_ci_cd_status" "$legacy_branding_status" "$legacy_runbooks_status" "$legacy_migrations_status" "$concepts_architecture_status" "$reference_self_stubs_status" "$guides_hygiene_status" <<'PY'
import json
import sys
from pathlib import Path
...
guides_hygiene_status = sys.argv[15]
...
overall_status = "pass"
if root_allowlist_violations > 0 or content_status != "pass" or legacy_arch_status != "pass" or legacy_ops_status != "pass" or legacy_ci_cd_status != "pass" or legacy_branding_status != "pass" or legacy_runbooks_status != "pass" or legacy_migrations_status != "pass" or concepts_architecture_status != "pass" or reference_self_stubs_status != "pass" or guides_hygiene_status != "pass":
    overall_status = "fail"
...
payload = {
    ...
    "guides_hygiene": guides_hygiene_status,
}
...
if [[ "$guides_hygiene_status" = "fail" ]]; then
  echo "Docs policy failed because docs/guides regressed into credential, stub, or docs-program clutter."
fi
if [[ "$status" = "fail" ]]; then
  exit 1
fi
