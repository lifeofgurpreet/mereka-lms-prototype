...
tools/docs/verify/verify-docs-scorecard-report-consistency.sh --report-glob "$OUT_FILE" >/tmp/generate_docs_scorecard_consistency_check.out 2>&1
tools/docs/verify/verify-docs-scorecard-report-timestamp.sh --report-glob "$OUT_FILE" >/tmp/generate_docs_scorecard_timestamp_check.out 2>&1
tools/docs/verify/verify-docs-scorecard-delta-artifact.sh \
...
grep -q "Command reference baseline coverage:" "$OUT_FILE"
grep -q "enabled=" "$OUT_FILE"
grep -q "entries=" "$OUT_FILE"
