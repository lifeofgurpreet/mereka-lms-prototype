...
bash tools/docs/verify/verify-docs-foundation-gates.sh \
  --summary-json "$FOUNDATION_SUMMARY" \
  --policy-range "${POLICY_RANGE:-HEAD...HEAD}" || true

bash tools/docs/verify/verify-doc-command-ref-baseline.sh \
...
bash tools/docs/verify/verify-doc-command-refs.sh \
  --include-baseline \
...
bash tools/docs/verify/verify-docs-scorecard-recency.sh \
  --max-age-days 7 \
  --summary-json "$SCORECARD_RECENCY_SUMMARY" || true

bash tools/docs/verify/verify-docs-scorecard-report-consistency.sh \
  --summary-json "$SCORECARD_CONSISTENCY_SUMMARY" || true

bash tools/docs/verify/verify-docs-scorecard-head-freshness.sh \
  --summary-json "$SCORECARD_HEAD_FRESHNESS_SUMMARY" || true

bash tools/docs/verify/verify-docs-scorecard-report-timestamp.sh \
  --summary-json "$SCORECARD_TIMESTAMP_SUMMARY" || true

bash tools/docs/verify/verify-docs-scorecard-delta-artifact.sh \
  --summary-json "$SCORECARD_DELTA_SUMMARY" || true

bash tools/docs/verify/verify-doc-link-integrity.sh \
...
|---|---|---|---|
| \`tools/docs/verify/verify-docs-foundation-gates.sh\` | ${dict[status_foundation_gates]:-unknown} | foundation summary | policy=${dict[status_foundation_policy]:-unknown}, repo_structure=${dict[status_foundation_repo_structure]:-unknown}, policy_content_consistency=${dict[status_foundation_policy_content_consistency]:-unknown}, policy_content_alignment=${dict[status_foundation_policy_content_alignment]:-unknown} |
...
| \`tools/docs/verify/verify-doc-command-ref-baseline.sh\` | ${dict[status_cmdref_baseline]:-unknown} | baseline summary | baseline file integrity contract, baseline_enabled=${dict[cmdref_baseline_enabled]:-false}, baseline_entries=${dict[cmdref_baseline_entries]:-0} |
| \`tools/docs/verify/verify-doc-command-refs.sh\` | ${dict[status_cmdref]:-unknown} | command refs summary | docs command/path references, files=${dict[files_checked]:-0}, candidates=${dict[total_candidates]:-0}, inline=${dict[candidates_inline_code]:-0}, shell=${dict[candidates_shell_block]:-0}, md_link=${dict[candidates_markdown_link]:-0}, md_autolink=${dict[candidates_markdown_autolink]:-0}, md_refdef=${dict[candidates_markdown_refdef]:-0}, missing_refs=${dict[missing_references]:-0} |
| \`tools/docs/verify/verify-doc-link-integrity.sh\` | ${dict[status_link_integrity]:-unknown} | link integrity summary | files=${dict[link_integrity_files_checked]:-0}, broken_links=${dict[link_integrity_broken_links]:-0} |
| \`tools/docs/verify/verify-docs-scorecard-recency.sh\` | ${dict[status_recency]:-unknown} | scorecard recency summary | max-age-days contract |
| \`tools/docs/verify/verify-docs-scorecard-report-consistency.sh\` | ${dict[status_consistency]:-unknown} | consistency summary | filename/title date contract |
| \`tools/docs/verify/verify-docs-scorecard-head-freshness.sh\` | ${dict[status_head_freshness]:-unknown} | head freshness summary | latest report aligned with HEAD date |
| \`tools/docs/verify/verify-docs-scorecard-report-timestamp.sh\` | ${dict[status_timestamp]:-unknown} | timestamp summary | Last verified (UTC) metadata contract |
| \`tools/docs/verify/verify-docs-scorecard-delta-artifact.sh\` | ${dict[status_delta]:-unknown} | delta artifact summary | quality/program date pairing contract |
| \`tools/docs/scorecards/build-docs-compliance-summary.py\` | ${dict[compliance_overall]:-unknown} | compliance summary | consolidated docs compliance status |
...
bash tools/docs/verify/verify-docs-scorecard-report-consistency.sh --report-glob "$REPORT_FILE" || true
bash tools/docs/verify/verify-docs-scorecard-report-timestamp.sh --report-glob "$REPORT_FILE" || true
bash tools/docs/verify/verify-docs-scorecard-delta-artifact.sh \
