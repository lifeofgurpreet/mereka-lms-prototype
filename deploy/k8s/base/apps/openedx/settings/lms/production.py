# -*- coding: utf-8 -*-
# ┌──────────────────────────────────────────────────────────────────────────────┐
# │ WARNING — SHADOW FILE. LMS RUNTIME DOES NOT LOAD THIS.                       │
# │                                                                              │
# │ The LMS pod loads settings from a K8s configmap mount at                     │
# │   /openedx/edx-platform/lms/envs/tutor/production.py                         │
# │ which is produced by kustomize `configMapGenerator` in the                   │
# │ bbi-infrastructure overlay. The authoritative source is:                     │
# │                                                                              │
# │   bbi-infrastructure/apps/mereka-lms/overlays/<env>/patches/production-<env>.py  │
# │   + sibling modules in bbi-infrastructure/apps/mereka-lms/overlays/<env>/    │
# │                                                                              │
# │ Edits to this app-repo file produce no runtime effect. Verifier greps        │
# │ against this file are NOT runtime proof (slices 63/64/67/68 evidence).       │
# │                                                                              │
# │ To change LMS runtime Django settings:                                       │
# │   - AUTHORITATIVE: edit the bbi-infra overlay file above                     │
# │   - Waffle flag (DB-backed): manage.py lms waffle_flag <name> --activate     │
# │   - Tutor plugin hook (image-baked only, NOT settings):                      │
...
# RETRACTED 2026-04-19 (slice 64): the MFE_CONFIG.setdefault for
# LEARNING_MICROFRONTEND_URL added by #1875 was a no-op. This shadow
# file is not loaded at runtime (see warning header); runtime proof:
#   kubectl exec deploy/lms -- python manage.py lms shell -c \
#     "from django.conf import settings; print(sorted([k for k in \
#      settings.MFE_CONFIG.keys() if 'LEARN' in k.upper()]))"
#   → ['LEARNER_HOME_MICROFRONTEND_URL', 'LEARNING_BASE_URL']
# (absence of LEARNING_MICROFRONTEND_URL confirms no-op).
# Evidence: docs/ops/evidence/shadow-settings-file-defect-2026-04-19.md
# Correct path: edit infrastructure/tutor/plugins/mereka_lms.py.
