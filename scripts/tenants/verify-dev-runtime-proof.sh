...
# ── Dev tenant table ─────────────────────────────────────────────────────────
# Dev is single-tenant — only mereka for now.
#
...
_check_image_for_deploy() {
  ...
  # Must not use :latest in dev (pinned SHA enforces image discipline)
  if [[ "$tag" == "latest" ]]; then
...
# ═══════════════════════════════════════════════════════════════════════════════
# SECTION 3 — Host acceptance (P0 dev hosts)
# ═══════════════════════════════════════════════════════════════════════════════
echo "--- [3] Host acceptance ---"

# Collect all P0 hosts for dev
...
printf '%s' "$HOST_ACCEPT_JSON" | python3 -m json.tool \
  ...
  || printf '%s' "$HOST_ACCEPT_JSON" > "$OUTPUT_DIR/dev-host-acceptance.json"
...
printf '%s' "$SC_DATA" | python3 -m json.tool \
  > "$OUTPUT_DIR/dev-siteconfig-proof.json" 2>/dev/null \
  || printf '%s' "$SC_DATA" > "$OUTPUT_DIR/dev-siteconfig-proof.json"
...
" 2>/dev/null || echo "null")
  ...
  # LMS_ROOT_URL should reference the dev LMS host
  ...
  # MFE_BASE_URL should reference the dev MFE host
...
studio_host = '$studio'
mfe_host = '$mfe'
primary_lms = 'academyv2.mereka.dev'   # Dev primary LMS host
primary_mfe = 'apps.academyv2.mereka.dev'  # Dev primary MFE host
...
" 2>/dev/null || echo "")
  ...
  case "$STATUS" in
    ok)
      pass_ "MFEConfig[$slug]: /api/mfe_config/v1 OK, no issues detected"
      ;;
    warning)
...
printf '%s' "$MFE_CONFIG_JSON" | python3 -m json.tool \
  > "$OUTPUT_DIR/dev-mfe-config-proof.json" 2>/dev/null \
  || printf '%s' "$MFE_CONFIG_JSON" > "$OUTPUT_DIR/dev-mfe-config-proof.json"
...
printf '%s' "$COOKIE_JSON" | python3 -m json.tool \
  > "$OUTPUT_DIR/dev-cookie-proof.json" 2>/dev/null \
  || printf '%s' "$COOKIE_JSON" > "$OUTPUT_DIR/dev-cookie-proof.json"
...
" 2>/dev/null || echo '{"status":"error"}')
  ...
  case "$STATUS" in
    correct)
      pass_ "AuthRedirect[$slug]: /login → $mfe/authn (correct MFE)"
      ;;
    no_redirect)
...
printf '%s' "$AUTH_REDIRECT_JSON" | python3 -m json.tool \
  > "$OUTPUT_DIR/dev-auth-redirect-proof.json" 2>/dev/null \
  || printf '%s' "$AUTH_REDIRECT_JSON" > "$OUTPUT_DIR/dev-auth-redirect-proof.json"
...
PROOF_FILE="$OUTPUT_DIR/dev-runtime-proof.json"
...
tenants_meta = [
    {'slug': 'mereka', 'lms': 'academyv2.mereka.dev', 'mfe': 'apps.academyv2.mereka.dev'},
]

host_data   = load_json(os.path.join(artifact_dir, 'dev-host-acceptance.json'))   or []
sc_data     = load_json(os.path.join(artifact_dir, 'dev-siteconfig-proof.json'))  or []
mfe_data    = load_json(os.path.join(artifact_dir, 'dev-mfe-config-proof.json'))  or []
cookie_data = load_json(os.path.join(artifact_dir, 'dev-cookie-proof.json'))      or []
auth_data   = load_json(os.path.join(artifact_dir, 'dev-auth-redirect-proof.json')) or []
