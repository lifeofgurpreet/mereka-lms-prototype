#!/usr/bin/env bash
# seed-prod-sites.sh — Idempotent seed/repair for Django Site + SiteConfiguration
#                     rows in the production namespace.
#
# Creates or updates Site and SiteConfiguration for each production tenant.
# Safe to re-run: uses get_or_create / update_or_create throughout.
#
# Usage:
#   scripts/tenants/seed-prod-sites.sh [--namespace NS] [--dry-run]
#
...
echo "=== seed-prod-sites: namespace=$NAMESPACE lms=$LMS_POD ==="
