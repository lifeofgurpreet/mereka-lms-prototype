...
# Verify `/api/mfe_config/v1` contract on the public MFE and LMS hosts.
#
# Why:
...
run_env() {
  ...
  echo "Checking:"
  echo "  - https://${mfe_domain}/api/mfe_config/v1?mfe=authn"
  echo "  - https://${lms_domain}/api/mfe_config/v1"
...
def load_json(url: str, label: str):
    try:
        with urllib.request.urlopen(url, timeout=30) as resp:
            raw = resp.read()
    except Exception as exc:
        fail(f"{label}: failed to fetch {url}: {exc}")

    try:
        return json.loads(raw.decode("utf-8"))
    except Exception as exc:
        sample = raw[:200].decode("utf-8", errors="replace")
        fail(f"{label}: invalid JSON response: {exc}; sample={sample!r}")
...
def validate_contract(label: str, payload: dict) -> None:
    global data
    data = payload
    ...
    if refresh not in (f"https://{mfe_domain}/login_refresh", "/login_refresh"):
        fail(
            f"{label}: REFRESH_ACCESS_TOKEN_ENDPOINT={refresh!r} "
...
mfe_url = f"https://{mfe_domain}/api/mfe_config/v1?mfe=authn"
lms_url = f"https://{lms_domain}/api/mfe_config/v1"

mfe_data = load_json(mfe_url, "MFE host")
validate_contract("MFE host", mfe_data)

lms_data = load_json(lms_url, "LMS host")
validate_contract("LMS host", lms_data)
...
for key in critical_keys:
    if lms_data.get(key) != mfe_data.get(key):
        fail(
            f"surface mismatch for {key}: "
            f"mfe={mfe_data.get(key)!r} lms={lms_data.get(key)!r}"
        )

print(f"[{env_name}] OK (MFE + LMS surfaces)")
PY
  then
    failures=$((failures + 1))
  fi
