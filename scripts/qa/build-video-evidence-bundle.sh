#!/usr/bin/env bash
# @covers AC-018
# @spec: video-pipeline-mux_spec.md
# Build a deterministic video evidence bundle for audits/incidents.
#
# Usage:
#   ./scripts/qa/build-video-evidence-bundle.sh
#   STRICT_RUNTIME=1 ./scripts/qa/build-video-evidence-bundle.sh --tar
#   ./scripts/qa/build-video-evidence-bundle.sh --out-dir var/video-evidence/manual-run
set -euo pipefail
...
OUT_DIR="${OUT_DIR:-var/video-evidence/${STAMP}}"
SKIP_CLUSTER=""

usage() {
  cat <<'USAGE'
Usage: ./scripts/qa/build-video-evidence-bundle.sh [--out-dir PATH] [--tar] [--skip-cluster]
Env:
...
echo "Build: Video evidence bundle"
echo "  out_dir:        $OUT_DIR"
echo "  context:        $K8S_CONTEXT"
echo "  strict_runtime: $STRICT_RUNTIME"
echo ""

run_check "verify-video-pipeline" \
  env STRICT_RUNTIME="$STRICT_RUNTIME" K8S_CONTEXT="$K8S_CONTEXT" \
  ./scripts/qa/verify-video-pipeline.sh

run_check "verify-video-observability" \
  env STRICT_RUNTIME="$STRICT_RUNTIME" K8S_CONTEXT="$K8S_CONTEXT" \
  ./scripts/qa/verify-video-observability.sh

run_check "verify-video-analytics" \
  env STRICT_RUNTIME="$STRICT_RUNTIME" K8S_CONTEXT="$K8S_CONTEXT" \
  ./scripts/qa/verify-video-analytics.sh

run_check "verify-video-upload" \
  env STRICT_RUNTIME="$STRICT_RUNTIME" K8S_CONTEXT="$K8S_CONTEXT" \
  ./scripts/qa/verify-video-upload.sh

run_check "verify-video-content-protection" \
  env STRICT_RUNTIME="$STRICT_RUNTIME" K8S_CONTEXT="$K8S_CONTEXT" \
  ./scripts/qa/verify-video-content-protection.sh

run_check "verify-mux-secrets" \
  env STRICT_RUNTIME="$STRICT_RUNTIME" K8S_CONTEXT="$K8S_CONTEXT" \
  ./scripts/qa/verify-mux-secrets.sh

run_check "verify-mux-alerts" \
  env STRICT_RUNTIME="$STRICT_RUNTIME" K8S_CONTEXT="$K8S_CONTEXT" \
