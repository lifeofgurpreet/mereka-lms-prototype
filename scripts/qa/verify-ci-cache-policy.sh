...
check_contains "$OPENEDX_BUILD" '--cache-from "type=gha,scope=${GHA_SCOPE}"' "build-openedx-image uses profile-scoped GHA cache restore"
check_contains "$OPENEDX_BUILD" '--cache-to "type=gha,mode=max,scope=${GHA_SCOPE}"' "build-openedx-image uses profile-scoped GHA cache write-back"
...
check_contains "$MFE_BUILD" '--cache-from "type=gha,scope=${GHA_SCOPE}"' "build-mfe-image uses profile-scoped GHA cache restore"
check_contains "$MFE_BUILD" '--cache-to "type=gha,mode=max,scope=${GHA_SCOPE}"' "build-mfe-image uses profile-scoped GHA cache write-back"
