...
BUDGET_DOC="$REPO_ROOT/docs/policies/architecture/PERFORMANCE_BUDGETS.md"
...
if [ -f "$CADDYFILE" ]; then
  if grep -qi "Cache-Control" "$CADDYFILE"; then
    ...
    do_warn "See: docs/policies/architecture/PERFORMANCE_BUDGETS.md section 'Caddy Configuration Requirements'"
  fi
else
