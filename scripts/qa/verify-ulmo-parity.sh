...
#   - MFE Dockerfile uses open-release/ulmo.1 for Atlas translations
...
if [[ ! -f "$MFE_DOCKERFILE" ]]; then
  ...
  ATLAS_ULMO=$(grep -c "revision=open-release/ulmo" "$MFE_DOCKERFILE" || true)
  if [[ "$ATLAS_ULMO" -ge 11 ]]; then
    pass "MFE Dockerfile: $ATLAS_ULMO Atlas translation pulls use open-release/ulmo.1"
  elif [[ "$ATLAS_ULMO" -ge 1 ]]; then
    fail "MFE Dockerfile: only $ATLAS_ULMO Atlas ulmo refs (expected >=11)"
  else
    fail "MFE Dockerfile: no open-release/ulmo Atlas translation refs"
  fi
