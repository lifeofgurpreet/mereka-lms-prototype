...
if [[ "$FAIL" -eq 0 ]]; then
  ...
  if [[ "$WARN" -gt 0 ]]; then
    ...
    echo "  - See docs/ops/runbooks/A11Y_CONTRAST_FOCUS_GATE.md for exception process"
  fi
  exit 0
else
  ...
  echo "See docs/ops/runbooks/A11Y_CONTRAST_FOCUS_GATE.md for the exception/reviewer process."
  exit 1
fi
