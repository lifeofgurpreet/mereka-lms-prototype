...
if [[ -f "$OPENEDX_DOCKERFILE" ]]; then
  ...
  elif grep -qE "pip install --no-build-isolation -r /tmp/base-filtered.txt -r /tmp/assets.txt" "$OPENEDX_DOCKERFILE" 2>/dev/null; then
    check_pass "Fallback pip install command (/tmp filtered requirements)"
  else
