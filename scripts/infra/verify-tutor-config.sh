...
if [[ -f "$OPENEDX_DOCKERFILE" ]]; then
  ...
  pattern_not_in_file 'build_profile_arg = "ARG MEREKA_BUILD_PROFILE=proof\n"' "$BUILD_OPTIMIZATIONS_SCRIPT" "Owner patch script does not retain render-owned build profile arg insertion"
  pattern_not_in_file 'Skipping translation refresh (fast build profile)' "$BUILD_OPTIMIZATIONS_SCRIPT" "Owner patch script does not retain render-owned translation refresh wrapper"
  ...
  pattern_not_in_file '$PIP_COMMAND install -r /openedx/edx-platform/requirements/edx/base.txt -r /openedx/edx-platform/requirements/edx/assets.txt' "$BUILD_OPTIMIZATIONS_SCRIPT" "Owner patch script does not retain base/assets uv no-build-isolation rewrite"
  ...
  pattern_not_in_file '$PIP_COMMAND install --no-cache-dir --compile uwsgi==2.0.24' "$BUILD_OPTIMIZATIONS_SCRIPT" "Owner patch script does not retain uwsgi uv-compile rewrite"
  ...
  pattern_not_in_file 'RUN ./manage.py lms --settings=tutor.i18n compilejsi18n\nRUN ./manage.py cms --settings=tutor.i18n compilejsi18n\n' "$BUILD_OPTIMIZATIONS_SCRIPT" "Owner patch script does not retain legacy compilejsi18n source rewrite"
  ...
  pattern_not_in_file 'RUN ./manage.py lms --settings=tutor.i18n compile_xblock_translations\nRUN ./manage.py cms --settings=tutor.i18n compile_xblock_translations\n' "$BUILD_OPTIMIZATIONS_SCRIPT" "Owner patch script does not retain legacy compile_xblock source rewrite"
  pattern_not_in_file 'RUN ./manage.py lms --settings=tutor.i18n compile_plugin_translations\n' "$BUILD_OPTIMIZATIONS_SCRIPT" "Owner patch script does not retain legacy compile_plugin source rewrite"
  ...
  pattern_not_in_file 'COPY --from=python-requirements --chown=app:app /openedx/openedx_advanced_xblocks /openedx/openedx_advanced_xblocks' "$BUILD_OPTIMIZATIONS_SCRIPT" "Owner patch script does not retain render-owned openedx_advanced_xblocks carry"
