...
# Repairs GKE (prod) MySQL auth drift by aligning MySQL users to the current
...
# - If upstream secrets drift (Infisical/GCP SM/ESO), services can fail with MySQL 1045.
#
# This script is non-destructive (no DB drops). It only runs ALTER USER/CREATE USER
# and CREATE DATABASE IF NOT EXISTS for optional services.
#
# Safety:
# - Does not print secret values.
# - Strips trailing CR/LF when reading secrets.
#
# Notes:
# - If root password is unknown due to prior drift, the script will search older
...
main() {
  ...
  # Optional services: ensure DBs + users exist, then align password.
