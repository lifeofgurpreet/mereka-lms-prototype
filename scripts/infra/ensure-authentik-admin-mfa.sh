...
CONFIRM_ENSURE_AUTHENTIK_ADMIN_MFA="${CONFIRM_ENSURE_AUTHENTIK_ADMIN_MFA:-}"
CONFIRM_TOKEN="ENSURE_AUTHENTIK_ADMIN_MFA"
...
Safety controls for --apply:
  CONFIRM_ENSURE_AUTHENTIK_ADMIN_MFA=ENSURE_AUTHENTIK_ADMIN_MFA
...
if [[ "$MODE" == "apply" ]]; then
  if [[ "$CONFIRM_ENSURE_AUTHENTIK_ADMIN_MFA" != "$CONFIRM_TOKEN" ]]; then
    echo "Refusing --apply without explicit confirmation token. Set CONFIRM_ENSURE_AUTHENTIK_ADMIN_MFA=${CONFIRM_TOKEN}" >&2
    exit 1
  fi
  ...
  if is_prod_like_context "$K8S_CONTEXT"; then
    if [[ "$CREATE_PREOP_BACKUP" == "1" ]]; then
      require_cmd velero
      backup_name="pre-op-${NAMESPACE}-ensure-authentik-admin-mfa-$(date -u +%Y%m%d-%H%M)"
