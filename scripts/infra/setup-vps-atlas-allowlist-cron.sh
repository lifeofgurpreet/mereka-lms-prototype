...
CRON_ENV_FILE="${CRON_ENV_FILE:-${REPO_ROOT}/var/atlas-allowlist-monitor.env}"
CRON_CMD="cd ${REPO_ROOT} && [ -f ${CRON_ENV_FILE} ] && . ${CRON_ENV_FILE}; ./scripts/infra/monitor-atlas-allowlist-vps.sh >> ${CRON_LOG} 2>&1"
...
log "Optional env file for webhook URL: ${CRON_ENV_FILE}"
