...
# DEPRECATED: legacy Tutor k8s override path.
# Canonical release flow:
#   scripts/infra/release-openedx-gitops.sh
# Temporary bypass (emergency only):
#   ALLOW_LEGACY_TUTOR_K8S=1 ./scripts/infra/setup-k8s-overrides.sh
if [[ "${ALLOW_LEGACY_TUTOR_K8S:-0}" != "1" ]]; then
  echo "DEPRECATED: scripts/infra/setup-k8s-overrides.sh is disabled by default." >&2
