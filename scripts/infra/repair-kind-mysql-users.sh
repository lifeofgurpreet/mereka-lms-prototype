...
CONFIRM_REPAIR_KIND_MYSQL_USERS="${CONFIRM_REPAIR_KIND_MYSQL_USERS:-}"
CONFIRM_TOKEN="REPAIR_KIND_MYSQL_USERS"
...
main() {
  ...
  if [[ "$CONFIRM_REPAIR_KIND_MYSQL_USERS" != "$CONFIRM_TOKEN" ]]; then
    echo "Refusing live mutation without explicit confirmation token. Set CONFIRM_REPAIR_KIND_MYSQL_USERS=${CONFIRM_TOKEN}" >&2
    exit 1
  fi
  ...
  if is_prod_like_context "$K8S_CONTEXT"; then
    if [[ "$CREATE_PREOP_BACKUP" == "1" ]]; then
      need_cmd velero
      backup_name="pre-op-${K8S_NAMESPACE}-repair-kind-mysql-users-$(date -u +%Y%m%d-%H%M)"
