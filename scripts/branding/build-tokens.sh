...
write_light_delta_theme() {
  ...
  # CSS custom properties via var(). light.min.css defines ALL token values
  # (~2000 tokens including dropdown-bg, modal shadows, etc). Without this file,
  # var() references resolve to transparent/initial → broken dropdowns, invisible text.
  #
  ...
  if [[ -f "$paragon_light_source" ]]; then
    cp "$paragon_light_source" "$target"
  else
