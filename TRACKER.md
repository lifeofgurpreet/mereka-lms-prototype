...
| T115 | Migrate legacy Content Libraries to new Libraries | P1 | DONE | Top50:#12 | L | T114 | No v1 libraries to migrate (content from Kajabi/MCT). `docs/ops/runbooks/CONTENT_LIBRARIES_V2_MIGRATION.md`, `scripts/qa/verify-content-libraries-v2.sh` (25P/0F/2SKIP). Dark launch ready, operator activates via feature flags. |
| T116 | Enterprise SSO: full OIDC/SAML + SCIM provisioning | P1 | DONE | Top50:#16 | L | T108 | Phase 0 complete: `docs/ops/runbooks/ENTERPRISE_SSO_GUIDE.md`, `scripts/qa/verify-enterprise-sso-readiness.sh` (13P/0F). ExternalSecret wired, SAML keypair gen ready. Phase 1 needs GCP secrets + first IdP config. |
